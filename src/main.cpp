...
// metra.cpp : Этот файл содержит функцию "main". Здесь начинается и заканчивается выполнение программы.
//
#include <iostream>
#include <math.h>
#include <cmath>
...
using namespace std;
constexpr complex<float> _i = { 0.0, 1.0 }; // или const для c++ < 11
...
int main()
{
	...
	vector <complex<float>> sgnl1;//ОПОРНЫЙ СИГНАЛ
	...
	vector <complex<float>> sgnl2;//ЗАШУМЛЕННАЯ РЕАЛИЗАЦИЯ СИГНАЛА
	...
	int SIZE = sgnl1.size();
	vector<complex<float>> cpysgnl2(sgnl2);//КОПИЯ ЗАШУМЛЕННОГО СИГНАЛА
	float masmax[3][401];
	...
	for (int f0 = -200; f0 <= 200; f0++) //ПОДСТРОЙКА ПО ЧАСТОТЕ
	{
		...
		vector <complex<float>> Fur1(sgnl1);//СОЗДАНИЕ ВЕКТОРА ДЛЯ БПФ ПЕРВОГО СИГНАЛА
		vector <complex<float>> Fur2(cpysgnl2);//СОЗДАНИЕ ВЕКТОРА ДЛЯ БПФ ВТОРОГО СИГНАЛА
		vector <complex<float>> Fur3(sgnl2);//СОЗДАНИЕ ВЕКТОРА ДЛЯ БПФ СВЕРТКИ ДВУХ СИГНАЛОВ
		//СОЗДАНИЕ ПЛАНОВ ПФ
		...
		fftw_execute(p2);//ВЫПОЛНЕНИЕ БПФ ВТОРОГО СИГНАЛА
